...
pub const ISSUER_SECURITY_DOMAIN_AID: [u8; 8] = [0xa0, 0x00, 0x00, 0x01, 0x51, 0x00, 0x00, 0x00];
...
pub struct AppletConfiguration {
    pub issuer_security_domain: bool,
    pub management: bool,
    pub hsmauth: bool,
    pub openpgp: bool,
    pub piv: bool,
...
impl AppletConfiguration {
    ...
    pub const fn contains(self, applet: Applet) -> bool {
        match applet {
            Applet::IssuerSecurityDomain => self.issuer_security_domain,
    ...
    pub const fn usb_capabilities(self) -> u16 {
        let ccid = if self.issuer_security_domain
            || self.management
            || self.hsmauth
            || self.openpgp
            || self.piv
...
impl OwnedCommandApdu {
    fn from_command(command: &CommandApdu<'_>) -> Self {
        Self {
            cla: command.cla,
            ins: command.ins,
            p1: command.p1,
    ...
    fn matches(&self, command: &CommandApdu<'_>) -> bool {
        self.cla == command.cla
            ...
            && self.le == command.le
            && self.extended == command.extended
    }
}
...
impl VirtualYubiKey {
    ...
    fn with_applets(
        ...
        let security_domain = security_domain::SecurityDomain::new(
            profile.serial,
            profile.firmware,
            profile.form_factor,
        );
    ...
    fn with_applets_and_security_domain(
        profile: DeviceProfile,
        piv: piv::PivApplet,
        hsmauth: hsmauth::HsmAuthApplet,
        fido: FidoAuthenticator,
        ...
        Self {
            ...
            secure_channel: secure_channel::SecureChannel::default(),
            security_domain,
            piv,
            hsmauth,
            fido,
    ...
    pub fn security_domain_persistent_state(&self) -> Result<Vec<u8>, &'static str> {
    ...
    /// Provision an SCP11a (0x11) or SCP11c (0x15) P-256 card key and host CA
    /// trust out of band. Versions must be nonzero; host CA IDs are 0x10 or
    /// 0x20..=0x2f. The private key uses software-key-core's serialized format.
    /// Trust certificates are DER and must include an explicitly trusted root.
    /// Replacing a key invalidates active secure channels. Persist the resulting
    /// Security Domain state using the same mechanism as the factory identity.
    pub fn provision_scp11(
        &mut self,
        kid: u8,
        kvn: u8,
        private_key: &[u8],
        host_kid: u8,
        host_kvn: u8,
        host_ca_certificates: &[Vec<u8>],
    ) -> Result<(), &'static str> {
        self.security_domain.provision_scp11(
            kid,
            kvn,
            private_key,
            host_kid,
            host_kvn,
            host_ca_certificates,
        )?;
    ...
    pub fn applet_for_aid(&self, aid: &[u8]) -> Option<Applet> {
        ...
        let candidates: [(Applet, &[u8]); 6] = [
            (Applet::IssuerSecurityDomain, &ISSUER_SECURITY_DOMAIN_AID),
    ...
    pub fn exchange_apdu(&mut self, raw: &[u8], presence: PresenceAuthorization) -> ApduExchange {
        ...
        // ISO chaining fragments the complete protected APDU. Reassemble before
        // checking its C-MAC; fragment acknowledgements carry no authorization.
        let assembled = match self.reassemble_command(command) {
        ...
        if !protected && command.cla == 0x80 && command.ins == 0x2a {
            let response = self
                .secure_channel
    ...
    fn dispatch_apdu(
        ...
        let response = if protected {
            self.secure_channel.protect_response(response)
        } else {
            response
        };
...
impl<'a> CommandApdu<'a> {
    pub fn decode(raw: &'a [u8]) -> Result<Self, ApduDecodeError> {
        ...
        let (data, le, extended) = match raw.len() {
            4 => (&raw[4..4], None, false),
            5 => (&raw[4..4], Some(short_le(raw[4])), false),
            _ if raw[4] != 0 => {
                let length = usize::from(raw[4]);
                let end = 5 + length;
                if raw.len() == end {
                    (&raw[5..end], None, false)
                } else if raw.len() == end + 1 {
                    (&raw[5..end], Some(short_le(raw[end])), false)
                } else {
...
mod tests {
    ...
    fn piv_and_hsmauth_persistence_are_independent() {
        ...
        let security_domain_encoded = device.security_domain_persistent_state().unwrap();
